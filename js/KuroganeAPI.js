...
class Move {
	constructor(api_id, hitbox_no, name, moveName, base_damage, angle, bkb, kbg, wbkb, hitboxActive, faf, landingLag, autoCancel, preDamage, counterMult, rehitRate, shieldDamage, weightDependent) {
		...
		this.windbox = this.name.includes("Windbox") || this.name.includes("Flinchless") || this.name == "Hydro Pump" || this.name == "F.L.U.D.D (Attack)" || (this.name == "PSI Magnet" && this.base_damage == 0);
		...
		this.updateMoveData = function () {
			...
			//Hardcoded fixes for KH stuff

			if (this.character == "Ness") {
				if (this.name == "PK Thunder (Tail)") {
					this.bkb = 20;
					this.kbg = 60;
					this.angle = 361;
				}
